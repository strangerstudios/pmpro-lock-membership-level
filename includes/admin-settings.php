...
function pmprolml_membership_level_before_content_settings( $level ) {
	...
	<div id="pmpro-lock-membership-level" class="pmpro_section" data-visibility="<?php echo esc_attr( $section_visibility ); ?>" data-activated="<?php echo esc_attr( $section_activated ); ?>">
		...
		<div class="pmpro_section_inside" <?php echo $section_visibility === 'hidden' ? 'style="display: none"' : ''; ?>>
			<p>
				<?php
				$lock_membership_link = '<a title="' . esc_attr__( 'Lock Membership Level Add On', 'pmpro-lock-membership-level' ) . '" target="_blank" rel="nofollow noopener" href="https://www.paidmembershipspro.com/add-ons/pmpro-lock-membership-level/?utm_source=plugin&utm_medium=pmpro-lock-membership-level&utm_campaign=add-ons&utm_content=&utm_term=">' . esc_html__( 'Lock Membership Level', 'pmpro-lock-membership-level' ) . '</a>';
				printf( esc_html__( 'Learn more about %s.', 'pmpro-lock-membership-level' ), $lock_membership_link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?>
			</p>
			<table>
