...
function pmprolml_getLevelOptions($level_id) {
	return get_option('pmprolml_level_' . intval($level_id) . '_settings', array('lock'=>0, 'expiration'=>'', 'expiration_number'=>'', 'expiration_period'=>''));
}
...
<tbody class="form-table">
	...
	<tr class="lml_expiration">
		...
		<td>
			<select id="lml_expiration" name="lml_expiration">
				<option value="" <?php if ( $options['expiration'] == '' ) echo " selected='selected'"; ?>><?php _e('Never', 'pmprolml');?></option>
				<option value="period" <?php if ( $options['expiration'] == 'period' ) echo " selected='selected'";?>><?php _e('Time Period', 'pmprolml');?></option>
			</select>
